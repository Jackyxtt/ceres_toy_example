...
struct Frame {
    ...
    unordered_map<int, Eigen::Vector3d> featurePerId; // 该帧观测到的特征(归一化平面上)以及特征id
};
...
//            vertexCam->SetFixed();
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        Eigen::Vector3d Pc = cameras[0].Rwc.transpose() * (Pw - cameras[0].twc);//在第一帧相机坐标系下的位姿
...
//        double inverse_depth = 1. / Pc.z();
    ...
    for(int i = 0; i < K; i++){
        ...
        edge_prior->SetVertex(edge_vertex);
